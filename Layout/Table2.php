			...
			<div class="col-sm-12">
				<table id="datatable" class="table" style="font-size: 14px">
					<thead>
                        ...
                        <?php
                            $sql = "SELECT *  FROM sinhvien WHERE MSSV like '%" . $_REQUEST['masv'] . "%' OR TenSV like '%" . $_REQUEST['masv'] . "%'"; 
                            if(! mysql_num_rows(mysql_query($sql)) )
                            {
                                die('Không tìm thấy sinh viên này vui lòng kiểm tra lại thông tin!' . mysql_error());
                            }
                            ...
                            $count = 0;
                            $retval = mysql_query($sql . " LIMIT " . $page*$length . "," . $length);
                            while ( $row = mysql_fetch_array($retval )) {
                                $count++;
                                $_sql = 'SELECT st.MSSV, TenSV, subj.MaMH, DiemGK FROM sinhvien st, monhoc subj, diem WHERE st.MSSV = diem.MSSV and subj.MaMH = diem.MaMH and ';
                            ?>
		...
		<ul id="pging_ul" style="float: right;">
			<?php
    			if(!empty($_REQUEST['page'])){
        			if($_REQUEST['page'] != 1){?>
        			    <li id="pging_li"><a href="./?frame=search&datatable_length=<?php echo $length;?>&masv=<?php echo $_REQUEST['masv'];?>&page=<?php echo ($_REQUEST['page'] - 1);?>"><?php echo '<<';?></a></li>
        			<?php }
                }
                $page = $i / $length;
                if($i % $length != 0){
                        ...
                        <li id="pging_li"> 
                            ...
                            if(!empty($_REQUEST['page'])){
                                if($int == $_REQUEST['page']){
                                 ...
                                 <a href="./?frame=search&datatable_length=<?php echo $length;?>&masv=<?php echo $_REQUEST['masv'];?>&page=<?php echo $int;?>"><?php echo $int;?></a>
                                <?php }
                            }else{
                             if($int == 1){
                                 echo '<span>' . $int . '</span>';
                ...
                if(!empty($_REQUEST['page'])){
                    if($_REQUEST['page'] != $page){ ?>
        			    <li id="pging_li"><a href="./?frame=search&datatable_length=<?php echo $length;?>&masv=<?php echo $_REQUEST['masv'];?>&page=<?php echo ($_REQUEST['page'] + 1);?>"><?php echo '<span>>></span>';?></a></li>
        			<?php }
                }else{
                    if($page != 1){ ?>
                    	<li id="pging_li"><a href="./?frame=search&datatable_length=<?php echo $length;?>&masv=<?php echo $_REQUEST['masv'];?>&page=<?php echo 2;?>"><?php echo '>>';?></a></li>
        		<?php }
                }
            ?>
		</ul>
