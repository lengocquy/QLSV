		...
		<ul id="pging_ul" style="float: right;">
			<?php
    			if(!empty($_REQUEST['page'])){
        			if($_REQUEST['page'] != 1){?>
        			    <li id="pging_li"><a href="./?frame=information&datatable_length=<?php echo $length;?>&masv=<?php echo $_REQUEST['masv'];?>&page=<?php echo ($_REQUEST['page'] - 1);?>"><?php echo '<<';?></a></li>
        			<?php }
                }
                $page = $i / $length;
                if($i % $length != 0){
                        ...
                        <li id="pging_li"> 
                            ...
                            if(!empty($_REQUEST['page'])){
                                if($int == $_REQUEST['page']){
                                 ...
                                 <a href="./?frame=information&datatable_length=<?php echo $length;?>&masv=<?php echo $_REQUEST['masv'];?>&page=<?php echo $int;?>"><?php echo $int;?></a>
                                <?php }
                            }else{
                             if($int == 1){
                                 echo '<span>' . $int . '</span>';
                ...
                if(!empty($_REQUEST['page'])){
                    if($_REQUEST['page'] != $page){ ?>
        			    <li id="pging_li"><a href="./?frame=information&datatable_length=<?php echo $length;?>&masv=<?php echo $_REQUEST['masv'];?>&page=<?php echo ($_REQUEST['page'] + 1);?>"><?php echo '<span>>></span>';?></a></li>
        			<?php }
                }else{
                    if($page != 1){ ?>
                    	<li id="pging_li"><a href="./?frame=information&datatable_length=<?php echo $length;?>&masv=<?php echo $_REQUEST['masv'];?>&page=<?php echo 2;?>"><?php echo '>>';?></a></li>
        		<?php }
                }
            ?>
		</ul>
