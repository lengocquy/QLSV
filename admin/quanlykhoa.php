<?php include_once('../config/config.php');
 	session_start();
		...
		<?php
			$qrkhoa = mysqli_query($conn, "SELECT `TenKhoa` FROM `khoa`");

			$qrlop = mysqli_query($conn, "SELECT `TenLop` FROM `lop`");

			$qrhk = mysqli_query($conn, "SELECT `id_hoc_ky` FROM `hocky`");

			$qrmon = mysqli_query($conn, "SELECT `TenMH` FROM `monhoc`");
		?>
